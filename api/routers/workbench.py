...
from portfolio_architect.claims.conversation import build_conversation_compare, _select_members, rank_members
...
_MAX_LIVE_MEMBERS = 12  # bound the synthesis prompt
...
# Controlled disease vocabulary for the dropdown — the showcase corpus is
# women's cancer drug evidence, so we match a small, curated set of cancer
# types against the freeform population/outcome/claim text rather than exposing
# thousands of raw population strings.
...
async def _load_cluster_centroids(
    ...
    for i in range(0, len(cluster_ids), CHUNK):
        ...
        for r in rows:
            out[r["cluster_id"]] = np.array(json.loads(r["claim_embedding"]), dtype=np.float32)
    return out


async def _cluster_disease_tags(conn, cluster_ids: list[str]) -> dict[str, list[str]]:
    """Map cluster_id -> the diseases ANY of its members touch (keyword match on
    member population/claim_text/outcome). Tagging from members, not just the
    cluster's single templated question, so a drug cluster spanning several
    cancers is highlightable under each — matching what the live generate path
    would return for that drug × disease. Reads only small columns stored before
    the embedding blob, index-backed via idx_claims_cluster → fast."""
...
async def list_clusters(
    ...
    if disease_tags is None or set(disease_tags) != set(cluster_ids):
        disease_tags = await _cluster_disease_tags(conn, cluster_ids)
...
async def cluster_detail(
    ...
    return {
        ...
        "citations_valid": cluster["citations_valid"],
...
async def workbench_options(project_id: UUID, conn=Depends(get_conn)):
    ...
    tags = await _cluster_disease_tags(conn, [c["id"] for c in multi])
    disease_counts: dict[str, int] = {k: 0 for k in DISEASES}
    for ds in tags.values():
        for d in ds:
            disease_counts[d] += 1
    diseases = [
        {"key": k, "label": DISEASES[k]["label"], "count": disease_counts[k]}
        for k in DISEASES if disease_counts[k]
    ]
...
async def single_claims(
    ...
    disease_meta = DISEASES.get(disease) if disease else None
...
async def generate_conversation(
    ...
    disease_meta = DISEASES.get(body.disease) if body.disease else None
    keywords = disease_meta["keywords"] if disease_meta else []
    ...
    # Prefer higher-confidence claims and cap the synthesis prompt.
    members.sort(key=lambda c: (c.get("confidence") or 0), reverse=True)
    members = members[:_MAX_LIVE_MEMBERS]
